...
module.exports = async (message) => {
    ...
    if (["BOŞBOŞBOBŞBOBŞOBŞBOŞ"].some(x => message.content === x)) {
        message.reply({ content:`İsim Taglarımız: \`${config.registration.GuilDTag}\`, \`${config.registration.GuilDTag2}\`, \`${config.registration.GuilDTag3}\`
Etiket Taglarımız: \`${config.registration.GuildDiscrim}\``}) 
    }
...
if (message.author.id === uye) {
    ...
    message.reply({ embeds: [afkembed.setDescription(`**AFK** modundan çıkış yaptınız.`)] }).then((e) => setTimeout(() => { e.delete(); }, 10000));
}
    ...
    const embed = new MessageEmbed().setColor(message.member.displayHexColor).setAuthor(message.member.displayName, author.avatarURL({ dynamic: true, size: 2048 })).setFooter("HydroniX 💛 1740")
